// ESTADO GLOBAL
function logDiagnostico(nivel, ...argumentos) {
  if (window.HBN1_DEBUG !== true) return;
  const metodo = ['log', 'warn', 'error'].includes(nivel) ? nivel : 'log';
...
const OPCOES_ORDENACAO = [
  { valor: 'padrao', label: 'PadrÃ£o' },
  { valor: 'desconto_desc', label: 'Maior desconto' },
  { valor: 'preco_asc', label: 'Menor preÃ§o' },
  { valor: 'preco_desc', label: 'Maior preÃ§o' },
  { valor: 'estoque_desc', label: 'Mais estoque' },
  { valor: 'alfabetica', label: 'Ordem alfabÃ©tica' },
];
// =========================================================================
// SUGESTÃƒO HIT â€” itens pendentes de positivaÃ§Ã£o do cliente Dedicado selecionado 
// =========================================================================
...
.catch(e => logDiagnostico('error', 'Erro ao carregar sugestÃ£o HIT:', e));
}
...
// Resolve TODOS os produtos das alternativas do grupo (nÃ£o sÃ³ o de maior estoque)
// â€” precisamos disso para detectar se o usuÃ¡rio jÃ¡ adicionou QUALQUER uma delas
// manualmente no catÃ¡logo, mesmo que nÃ£o seja a que o sistema sugeriu.
let melhor = null;
...
produtosDoGrupo // todas as alternativas "OU" jÃ¡ resolvidas no catÃ¡logo atual
});
}
});
});
}
// Verifica se QUALQUER alternativa ("OU") do grupo jÃ¡ estÃ¡ no carrinho.
...
// Conta sÃ³ o que REALMENTE ainda falta â€” itens jÃ¡ satisfeitos por alguma
// alternativa do OU nÃ£o entram no contador.
...
// Alguma alternativa do OU jÃ¡ estÃ¡ no carrinho â€” mostra como concluÃ­do,
// usando o produto REALMENTE adicionado (pode nÃ£o ser o "melhor" sugerido)
if (produtoNoCarrinho) {
const imagemCarrinho = urlImagemSegura(produtoNoCarrinho.imagens);
return `
       <div class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-2.5">
         <img src="${imagemCarrinho}" alt="" class="w-12 h-12 object-contain bg-white rounded-lg p-1 border border-emerald-100 shrink-0 mix-blend-multiply">
         <div class="min-w-0 flex-grow">
           <span class="text-[8px] font-black text-purple-600 uppercase tracking-wider">${escapeHtml(alavanca)}</span>
           <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(produtoNoCarrinho.descricao || produtoNoCarrinho.id)}</p>
           <p class="text-[10px] text-emerald-600 font-bold">Já no pedido (${Number(CARRINHO[produtoNoCarrinho.id]) || 0} un)</p>
         </div>
       </div>`;
}

const { precoFinal, precoOriginal } = calcularPrecos(produto);
const imagemProduto = urlImagemSegura(produto.imagens);
...
return `
     <div class="flex items-center gap-3 bg-white border border-slate-100 rounded-xl p-2.5">
       <img src="${imagemProduto}" alt="" class="w-12 h-12 object-contain bg-slate-50 rounded-lg p-1 border border-slate-100 shrink-0 mix-blend-multiply">
       <div class="min-w-0 flex-grow">
         <span class="text-[8px] font-black text-purple-600 uppercase tracking-wider">${escapeHtml(alavanca)}</span>
         <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(produto.descricao || item.descricao || item.cod)}</p>
         <p class="text-[10px] text-slate-400 font-mono">Cód ${escapeHtml(item.cod)} · EAN ${escapeHtml(item.ean)}</p>
       </div>
...
if (_produtoDoGrupoNoCarrinho(itemHit)) return; // jÃ¡ satisfeito por alguma alternativa do OU
...
if (adicionados > 0) {
mostrarToast('success', `${adicionados} item(ns) da sugestÃ£o HIT adicionados ao pedido.`);
} else {
mostrarToast('warning', 'Nenhum item pÃ´de ser adicionado (sem estoque disponÃ­vel ou jÃ¡ no pedido).');
}
}
// =========================================================================
// ESCOLHA MANUAL HIT â€” vendedor define quantidades por produto, por grupo
// =========================================================================
let HIT_MANUAL_QTDS = {}; // { produtoId: qtd }

function abrirEscolhaManualHit() {
  if (TIPO_USUARIO === 'VENDEDOR_FARMA') { mostrarToast('warning', 'FunÃ§Ã£o disponÃ­vel apenas para equipe Dedicado.'); return; }
  if (!CLIENTE_SELECIONADO) { mostrarToast('warning', 'Selecione um cliente antes.'); return; }
  const equipe = String(CLIENTE_SELECIONADO.equipe || '').toUpperCase().trim();
  if (equipe !== 'DEDICADO') { mostrarToast('warning', 'Esta funÃ§Ã£o Ã© exclusiva para clientes com equipe Dedicado.'); return; }
  if (!HIT_DADOS_CLIENTE) {
    mostrarToast('info', 'Carregando dados HIT do cliente...');
    carregarSugestaoHit();
    setTimeout(() => {
      if (HIT_DADOS_CLIENTE) abrirEscolhaManualHit();
      else mostrarToast('error', 'NÃ£o hÃ¡ campanha HIT ativa para este cliente.');
    }, 900);
    return;
  }
...
function renderizarEscolhaManualHit() {
  ...
  if (!HIT_DADOS_CLIENTE || !HIT_DADOS_CLIENTE.alavancas || HIT_DADOS_CLIENTE.alavancas.length === 0) {
    corpo.innerHTML = '<div class="text-center py-10 text-slate-400 text-sm font-medium">Nenhuma alavanca disponÃ­vel para este cliente.</div>';
    return;
  }
  ...
  corpo.innerHTML = HIT_DADOS_CLIENTE.alavancas.map(al => {
        ...
        return `
          <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[8px] font-black text-slate-400 uppercase tracking-wider">${g.itens.length > 1 ? 'OU â€” Qualquer um destes' : 'SKU Ãºnico'}</span>
              <span class="text-[9px] font-black text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full">âœ… Positivado</span>
            </div>
            <p class="text-[11px] font-bold text-slate-700">${escapeHtml(item.descricao || item.cod || '')}</p>
          </div>`;
      }
      ...
      if (itensResolvidos.length === 0) {
        return `
          <div class="bg-slate-100 border border-slate-200 rounded-xl p-3">
            <span class="text-[8px] font-black text-slate-400 uppercase tracking-wider block mb-1">${g.itens.length > 1 ? 'OU â€” Qualquer um destes' : 'SKU Ãºnico'}</span>
            <p class="text-[11px] text-slate-400 italic">Nenhuma opÃ§Ã£o disponÃ­vel no catÃ¡logo atual.</p>
          </div>`;
      }
      ...
      const linhasItens = itensResolvidos.map(({ item, produto }) => {
        ...
        return `
          <div class="flex items-center gap-2.5 bg-white border border-slate-100 rounded-xl p-2">
            <img src="${urlImagemSegura(produto.imagens)}" alt="" class="w-10 h-10 object-contain bg-slate-50 rounded-lg p-1 border border-slate-100 shrink-0 mix-blend-multiply">
            <div class="min-w-0 flex-grow">
              <p class="text-[11px] font-bold text-slate-800 truncate">${escapeHtml(produto.descricao || item.cod)}</p>
              <p class="text-[9px] text-slate-400 font-mono">Cód ${escapeHtml(item.cod)} · Est: ${estoque} · ${formatarParaReal(precoFinal)}</p>
            </div>
            <div id="manualqtd-${escapeHtml(produto.id)}" class="shrink-0">
      ...
      return `
        <div class="bg-white border border-slate-200 rounded-xl p-3">
          <div class="flex items-center justify-between mb-2">
            <span class="text-[8px] font-black text-slate-400 uppercase tracking-wider">${g.itens.length > 1 ? 'OU â€” Qualquer um destes' : 'SKU Ãºnico'}</span>
            <span class="text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-full">â€” Pendente</span>
          </div>
    ...
    return `
      <div>
        <div class="flex items-center justify-between mb-2">
          <span class="text-xs font-black text-purple-700 uppercase tracking-wider">${escapeHtml(al.alavanca)}</span>
...
function _htmlStepperManualHit(produtoId, qtd, estoqueMax, disabled) {
  const produtoIdJs = escapeJsString(produtoId);
  ...
  if (qtd > 0) return `
    <div class="flex items-center bg-purple-50 border border-purple-200 rounded-lg p-0.5">
      <button onclick="alterarQtdManualHit('${produtoIdJs}', -1, ${estoqueMax})" class="w-6 h-6 bg-white hover:bg-purple-100 rounded font-bold text-xs text-purple-600 flex items-center justify-center border border-purple-100">âˆ’</button>
      <input type="number" value="${qtd}" min="1" max="${estoqueMax}" onchange="atualizarQtdManualHitDigitada('${produtoIdJs}', this.value, ${estoqueMax})" onkeydown="if(event.key==='Enter') this.blur();"
        class="w-9 text-center font-black text-xs text-purple-700 bg-transparent focus:outline-none p-0 border-0">
      <button onclick="alterarQtdManualHit('${produtoIdJs}', 1, ${estoqueMax})" class="w-6 h-6 bg-white hover:bg-purple-100 rounded font-bold text-xs text-purple-600 flex items-center justify-center border border-purple-100">+</button>
    </div>`;
  return `<button onclick="alterarQtdManualHit('${produtoIdJs}', 1, ${estoqueMax})" class="px-2.5 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-[10px] font-bold rounded-lg transition-all">+ Add</button>`;
}

function alterarQtdManualHit(produtoId, delta, estoqueMax) {
  let qtd = (HIT_MANUAL_QTDS[produtoId] || 0) + delta;
  if (qtd > estoqueMax) { mostrarToast('warning', `MÃ¡ximo em estoque: ${estoqueMax} un.`); qtd = estoqueMax; }
...
function adicionarSelecaoManualHit() {
  ...
  else mostrarToast('warning', 'Nenhum item pÃ´de ser adicionado.');
}

// =========================================================================
// SUGESTÃƒO DE PEDIDO â€” baseada no histÃ³rico do Fat. Tri (cliente ou grupo)
// =========================================================================
let SUGESTAO_PEDIDO_DADOS   = null; // resposta bruta da API
let SUGESTAO_PEDIDO_ITENS   = [];   // [{ produto, freq, score, valorSugerido }] jÃ¡ casados com BD_PRODUTOS e montados atÃ© o valor alvo
...
//function carregarSugestaoPedido() {
  ...
  //.then(resp => {
   ...
   // montarItensSugestaoPedido();
    //atualizarBotaoSugestaoPedido();
 // })
//  .catch(e => logDiagnostico('error', 'Erro ao carregar sugestÃ£o de pedido:', e));
//}
...
function montarItensSugestaoPedido() {
  ...
  // 1) Seleciona os itens mais relevantes (limite razoÃ¡vel pra nÃ£o pulverizar demais)
  const candidatos = [];
  for (const item of SUGESTAO_PEDIDO_DADOS.itens) {
    ...
    if (candidatos.length >= 12) break; // top 12 jÃ¡ Ã© uma cesta bem representativa
  }

  if (candidatos.length === 0) return;

  // 2) Distribui o valor alvo proporcionalmente ao peso histÃ³rico de cada item
...
if (SUGESTAO_PEDIDO_DADOS.temHistorico) {
  ...
  if (meses > 0) {
    origemTxt = `Com base no histÃ³rico de compras do cliente â€” mÃ©dia calculada sobre ${meses} ${meses === 1 ? 'mÃªs fechado' : 'meses fechados'}`;
  } else {
    origemTxt = `Cliente novo neste fornecedor (sÃ³ hÃ¡ compras no mÃªs em andamento) â€” usando piso de R$ 1.000`;
  }
} else {
  origemTxt = `Sem histÃ³rico neste fornecedor â€” sugestÃ£o baseada no mix mais comprado pela equipe ${SUGESTAO_PEDIDO_DADOS.equipe || ''}`;
}
document.getElementById('sugestaoPedidoOrigem').innerText = origemTxt;
  ...
  corpo.innerHTML = SUGESTAO_PEDIDO_ITENS.map(({ produto, qtd, precoFinal, valorItem, freq }) => {
    ...
    return `
       <div class="flex items-center gap-3 bg-white border border-slate-100 rounded-xl p-2.5">
         <img src="${urlImagemSegura(produto.imagens)}" alt="" class="w-12 h-12 object-contain bg-slate-50 rounded-lg p-1 border border-slate-100 shrink-0 mix-blend-multiply">
         <div class="min-w-0 flex-grow">
           <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(produto.descricao || produto.id)}</p>
           <p class="text-[10px] text-slate-400 font-mono">Qtd sugerida: ${qtd} un ${freq ? `â€¢ Comprado ${freq}x no perÃ­odo` : ''}</p>
         </div>
         <div class="text-right shrink-0">
...
function adicionarItensSugestaoPedido() {
  ...
  if (adicionados > 0) {
    mostrarToast('success', `${adicionados} item(ns) da sugestÃ£o de pedido adicionados.`);
  } else {
    mostrarToast('warning', 'Nenhum item pÃ´de ser adicionado (sem estoque ou jÃ¡ no pedido).');
  }
}

// -----------------------------------------------------------------------
// DARK MODE â€” aplicado antes de qualquer render
// -----------------------------------------------------------------------
function toggleDarkMode() {
  ...
  // MantÃ©m as transiÃ§Ãµes desligadas atÃ© o novo tema ter sido pintado.
...
function atualizarControleTema(tema) {
  ...
  if (icon) icon.innerText = escuro ? 'â˜€ï¸' : 'ðŸŒ™';
  if (botao) {
...
try {
  ...
  // A limpeza local acontece mesmo quando a rede está indisponível.
} finally {
  limparDadosDaSessao();
...
// Fallback para instalaÃ§Ãµes antigas; o HTML jÃ¡ aplica o tema no <head>.
(function() {
...
// UF do usuÃ¡rio logado: vem da URL (?uf=...) ou do localStorage (login feito em index.html)
const params = new URLSearchParams(window.location.search);
let UF_USUARIO = (params.get('uf') || sessionStorage.getItem('hbn1_uf') || 'PI').toUpperCase();

// Sem usuÃ¡rio logado? manda de volta para o login â€” exceto se a URL tiver
// ?oferta=TOKEN, caso em que quem decide o que mostrar Ã© o script de
// bootstrap da oferta compartilhada (ver <head> do catalogo.html).
if (!sessionStorage.getItem('hbn1_usuario') && !params.get('oferta')) {
window.location.href = 'index.html';
}
let UFS_PERMITIDAS_USUARIO = [];
try { UFS_PERMITIDAS_USUARIO = JSON.parse(sessionStorage.getItem('hbn1_ufs') || '[]'); } catch(e) {}
if (UFS_PERMITIDAS_USUARIO.length === 0) UFS_PERMITIDAS_USUARIO = [UF_USUARIO];
// =========================================================================
// Produtos não são persistidos no navegador. Isso evita deixar preços,
// estoques e regras comerciais expostos no painel Application após o uso.
// =========================================================================
function _chaveCacheProdutos() {
  return 'hbn1_cache_produtos_' + String(UF_USUARIO || 'PI').toUpperCase();
}

function _salvarCacheProdutos() {
  // Intencionalmente vazio: dados comerciais permanecem apenas em memória.
}

function _carregarCacheProdutos() {
  try { sessionStorage.removeItem(_chaveCacheProdutos()); } catch (_) {}
  CACHE_PRODUTOS_TIMESTAMP = null;
  return null;
}
  ...
  const trocar = () => {
    chamarApi('trocarUf', { uf: novaUf }).then(resp => {
      if (resp && resp.sucesso) {
        sessionStorage.setItem('hbn1_uf', novaUf);
        window.location.href = 'catalogo.html?uf=' + novaUf;
      } else {
        mostrarToast('error', (resp && resp.mensagem) || 'NÃ£o foi possÃ­vel trocar de UF.');
      }
    }).catch(() => mostrarToast('error', 'Erro ao trocar de UF.'));
  };
  if (Object.keys(CARRINHO).length > 0) {
    mostrarConfirm('Trocar de UF?', 'VocÃª tem itens no pedido atual. Ao trocar de UF o carrinho serÃ¡ limpo. Deseja continuar?', trocar);
  } else {
    trocar();
  }
}

// Tipo do usuÃ¡rio logado (PROMOTOR / VENDEDOR_FARMA / VENDEDOR_DEDICADO / ADMIN).
// Compatibilidade: cadastros antigos com "VENDEDOR" puro contam como Farma.
let TIPO_USUARIO = (sessionStorage.getItem('hbn1_tipo') || 'VENDEDOR_FARMA').toUpperCase();
...
async function aguardarSessaoInicial() {
  try {
    ...
    alert(erro.message || 'NÃ£o foi possÃ­vel abrir o link da oferta.');
    window.location.replace('index.html');
    throw erro;
  }

  // Releia depois do bootstrap, pois a oferta acabou de gravar o sessionStorage.
  UF_USUARIO = (params.get('uf') || sessionStorage.getItem('hbn1_uf') || 'PI').toUpperCase();
  TIPO_USUARIO = (sessionStorage.getItem('hbn1_tipo') || 'VENDEDOR_FARMA').toUpperCase();
  if (TIPO_USUARIO === 'VENDEDOR') TIPO_USUARIO = 'VENDEDOR_FARMA';
  try { UFS_PERMITIDAS_USUARIO = JSON.parse(sessionStorage.getItem('hbn1_ufs') || '[]'); }
  catch (_) { UFS_PERMITIDAS_USUARIO = []; }
  if (UFS_PERMITIDAS_USUARIO.length === 0) UFS_PERMITIDAS_USUARIO = [UF_USUARIO];
}

// Perfis autorizados a usar o TalÃ£o em branco (Excel). Pra liberar geral,
...
// Valores mÃ­nimos por grupo de desconto
let BD_VALORES_MINIMOS = {}; // { chave: { minimo, label } }
let BD_CHAVES_VENCIDAS = new Set(); // chaves de desconto cuja validade (coluna D) jÃ¡ passou

// ST (SubstituiÃ§Ã£o TributÃ¡ria): mapa { id_produto -> valor_st_em_reais }
let BD_ST = {};        // carregado da API
let ST_ATIVO = false;  // controlado pelos botÃµes
// CONFIGURAÃ‡ÃƒO DE FORNECEDORES â€” cores, logos, gradientes
...
// Logo HBN/NazÃ¡ria (fundo transparente) usada no cabeÃ§alho do PDF do catÃ¡logo (baixarCatalogoPdf).
...
// =========================================================================
// SPLASH DE ABERTURA (estilo Netflix) â€” nome + logos dos fornecedores
// =========================================================================
...
// Entrada escalonada dos logos â€” comeÃ§a logo apÃ³s o nome aparecer
requestAnimationFrame(() => {
...
// Fecha a splash e revela a tela de portais (que jÃ¡ estÃ¡ carregando por trÃ¡s)
...
// =========================================================================
// SISTEMA DE TOAST / NOTIFICAÃ‡Ã•ES (substitui alert() nativo)
// =========================================================================
let _TOAST_ID = 0;
...
if (!container) { logDiagnostico('warn', 'Toast container nÃ£o encontrado:', mensagem); return; }
...
// =========================================================================
// MODAL DE CONFIRMAÃ‡ÃƒO (substitui confirm() nativo)
// =========================================================================
...
// Fundo jÃ¡ Ã© branco fixo via CSS
// Nome do usuÃ¡rio
document.getElementById('portaisNomeUsuario').innerText =
sessionStorage.getItem('hbn1_nome') || sessionStorage.getItem('hbn1_usuario') || '';
...
// Fornecedores com produtos vÃ¡lidos
...
// Skeleton shimmer no formato exato dos cards finais â€” evita texto solto e layout shift
...
return `
         <button onclick="entrarFornecedor('${forn}')" data-portal-card
           ...
           <span class="text-white/85 text-[10px] font-bold uppercase tracking-wider">${qtd} produto${qtd !== 1 ? 's' : ''} disponÃ­ve${qtd !== 1 ? 'is' : 'l'}</span>
...
// Entrada escalonada (stagger) â€” cada card surge ~60ms depois do anterior
requestAnimationFrame(() => {
...
atualizarEstiloBotaoFiltro('btnFiltroDivisao', false, 'DivisÃ£o');       
sessionStorage.setItem('hbn1_fornecedor_ativo', filtroFornecedorAtual);
...
// AmbientaÃ§Ã£o: header do fornecedor
...
// Atualiza painÃ©is OL conforme o fornecedor
...
sessionStorage.removeItem('hbn1_fornecedor_ativo');
atualizarBotaoSugestaoHit();
mostrarTelaPortais();
}
...
// SubstituiÃ§Ã£o de renderizarFiltrosLaterais â€” agora atualiza os portais
function renderizarFiltrosLaterais() {
// Mantida para compatibilidade â€” agora sÃ³ atualiza a tela de portais
...
// CÃLCULO DE PREÃ‡O COM DESCONTO DINÃ‚MICO (versÃ£o genÃ©rica)
// Aceita cliente e OL explÃ­citos â€” usada tanto pelo catÃ¡logo (globais)
// quanto pela importaÃ§Ã£o de pedidos (cliente identificado pelo CNPJ)
...
// As regras antigas sÃ³ executam se o cliente nÃ£o recebeu desconto por grupo.
if (percentual === 0) {  
...
// NOVO â€” fallback: se a regra especÃ­fica do fornecedor nÃ£o gerou desconto
// (coluna vazia ou perfil/equipe do cliente nÃ£o bate com nenhum critÃ©rio),
// tenta o Desconto PadrÃ£o antes de zerar de vez. SÃ³ entra aqui se a coluna
// especÃ­fica ficou em 0 â€” nunca sobrescreve um desconto especÃ­fico jÃ¡ achado.
...
// Desconto vinculado a uma oferta vencida (coluna D da VALOR MINIMO) â€” zera
if (colunaAtiva && BD_CHAVES_VENCIDAS.has(colunaAtiva)) {
percentual = 0;
colunaAtiva = null;
}

// SeguranÃ§a comercial: desconto nunca pode aumentar o preÃ§o nem gerar valor negativo.
...
// Centraliza o preÃ§o efetivamente exibido e utilizado pelo pedido.
// A ST Ã© somada Ã  base antes da aplicaÃ§Ã£o do desconto, seguindo a regra
// comercial jÃ¡ mostrada nos cards. Todos os totais, histÃ³rico e exportaÃ§Ãµes
...
// Converte "15%", "15", 15, "15,5%" etc. â†’ nÃºmero float
...
if (/^data:image\/(?:png|jpeg|jpg|webp|gif);base64,/i.test(url)) return escapeHtml(url);
if (/^(?:\.{0,2}\/)[A-Za-z0-9_./%-]+$/i.test(url)) return escapeHtml(url);
return '';
}

// TEXTO DE ÃšLTIMA ATUALIZAÃ‡ÃƒO DOS DADOS (produtos/preÃ§os/estoque)
...
el.innerText = origem === 'cache'
  ? `âš  Dados em cache de ${dataFormatada} Ã s ${horaFormatada} â€” sincronizando`
  : `âœ“ Dados atualizados em ${dataFormatada} Ã s ${horaFormatada}`;
...
// =========================================================================
// SELEÃ‡ÃƒO DE OL DANONE / OMRON (inline na barra de cliente)
// =========================================================================
...
atualizarIndicadoresFinanceirosGlobais();
atualizarResumoValoresMinimos();
// Atualiza indicador de mÃ­nimos na barra
atualizarIndicadorMinimosBarra();
}
...
// Separador: visÃ­vel se cliente selecionado E pelo menos um painel de OL visÃ­vel
...
// Popover â€” badges de perfil
const badgesEl = document.getElementById('popCliBadges');
const badges = [
cliente.grupoUnilever
? { txt: 'ðŸ”µ UNI: ' + cliente.grupoUnilever,  cls: 'bg-blue-900/60 text-blue-200 border-blue-800' } : null,
cliente.perfilDanone
? { txt: 'ðŸŸ£ DAN: ' + cliente.perfilDanone,   cls: 'bg-purple-900/60 text-purple-200 border-purple-800' } : null,
String(cliente.painelTransfer || '').toUpperCase() === 'TRANSFER KENVUE'
? { txt: 'ðŸŸ  KEN: TRANSFER', cls: 'bg-orange-900/60 text-orange-200 border-orange-800' } : null,
cliente.equipe
? { txt: 'ðŸŸ¢ EQ: ' + cliente.equipe, cls: 'bg-emerald-900/60 text-emerald-200 border-emerald-800' } : null,
].filter(Boolean);
...
// Recalcula preÃ§os com novo cliente e atualiza painÃ©is OL
executarFiltrosGerais();
atualizarIndicadoresFinanceirosGlobais();
...
// Fecha popover de mÃ­nimos ao clicar fora
...
// Base sempre pelo fornecedor ativo (nÃ£o pelos outros filtros), pra lista
// de opÃ§Ãµes nÃ£o sumir conforme o usuÃ¡rio vai filtrando
...
function renderizarDropdownDivisao() {
  ...
  if (divisoes.length === 0) {
    dd.innerHTML = `<div class="px-3 py-2 text-[11px] text-slate-400 italic">Sem divisÃµes cadastradas</div>`;
    return;
  }
  let html = `<button onclick="selecionarFiltroDivisao(null)" class="w-full text-left px-3 py-2 text-[11px] font-bold ${!FILTRO_DIVISAO_ATIVA ? 'text-orange-600 bg-orange-50' : 'text-slate-600 hover:bg-slate-50'}">Todas as divisÃµes</button>`;
  html += divisoes.map(d => `
...
function selecionarFiltroDivisao(divisao) {
  ...
  atualizarEstiloBotaoFiltro('btnFiltroDivisao', !!divisao, divisao || 'DivisÃ£o');
  executarFiltrosGerais();
}
...
function aplicarOrdenacaoAtual() {
  switch (ORDENACAO_ATIVA) {
    ...
    case 'alfabetica':
      ...
      break;
    default:
      break; // mantÃ©m a ordem original (a que jÃ¡ vem de BD_PRODUTOS)
  }
}
...
// Indicadores no canto (carrinho + ST) â€” ficam num container com id prÃ³prio,
...
: `<div>
            <p class="text-base font-black text-slate-900 leading-none">${precoExibidoOriginal > 0 ? formatarParaReal(precoExibidoOriginal) : 'â€”'}</p>
          </div>`;
...
card.innerHTML = `
       ...
       <div class="p-2.5 flex flex-col flex-grow gap-1.5">
         ...
         <p class="text-[9px] text-slate-400 font-mono">
           Emb. <span class="text-slate-500 font-semibold">${p.embalagem ? escapeHtml(p.embalagem) + ' un' : 'â€”'}</span>
           <span class="mx-1">Â·</span>
           ID <span class="text-slate-500 font-semibold">${escapeHtml(p.id)}</span>
         </p>

         <!-- PreÃ§o â€” hero do card -->
         <div class="mt-auto pt-2 border-t border-slate-100">
           <div class="flex items-end justify-between mb-2">
             ${blocoPrecoHtml}
            ${temDesconto ? `<span class="badge-desconto-forte text-white text-[10px] font-black px-2 py-0.5 rounded-lg">âˆ’${percentual}%</span>` : ''}
           </div>
...
if (qtd > 0) return `
       <div class="flex items-center justify-between bg-orange-50 border border-orange-200 rounded-xl p-0.5 w-full">
         <button onclick="alterarQtd('${idJs}', -1)" aria-label="Diminuir quantidade" class="w-7 h-7 bg-white hover:bg-orange-100 rounded-lg font-bold text-sm text-orange-500 flex items-center justify-center border border-orange-100 transition-colors shrink-0">âˆ’</button>
...
// Monta o HTML dos indicadores no canto do card (Ã­cone de carrinho + bolinha de ST).
// Isolado numa funÃ§Ã£o para poder ser chamado tanto na criaÃ§Ã£o do card quanto
// na atualizaÃ§Ã£o de quantidade (sem precisar recriar o card inteiro).
...
if (novaQtd > estoqueMax) { mostrarToast('warning', `Quantidade mÃ¡xima em estoque atingida (${estoqueMax} un).`); novaQtd = estoqueMax; }
...
if (isEsgotado) return; // esgotado nunca entra no carrinho, mantÃ©m estilo
card.className = "bg-white border rounded-2xl flex flex-col overflow-hidden relative group text-[11px] transition-all duration-200 " +
(noCarrinho
? "border-orange-300 shadow-md shadow-orange-100"
: "border-slate-100 hover:border-orange-300 hover:shadow-xl hover:shadow-orange-100/60 hover:-translate-y-1");

// Recalcula o Ã­cone de carrinho / bolinha de ST no canto, jÃ¡ que o card
// nÃ£o Ã© recriado do zero ao sÃ³ mudar a quantidade.
...
// Badges do menu lateral (catÃ¡logo + portais)
...
// Compatibilidade com versÃµes anteriores que ainda tenham o botÃ£o flutuante de salvamento.
...
mostrarConfirm(
'Limpar pedido?',
'Tem certeza que deseja limpar todos os produtos selecionados do seu pedido? Esta aÃ§Ã£o nÃ£o pode ser desfeita.',
() => {
...
if (chaves.length === 0) {
boxCorpo.innerHTML = `<div class="text-center py-12 text-slate-400 font-medium"><p class="text-base font-bold">Seu pedido estÃ¡ vazio.</p><p class="text-xs mt-1">Adicione quantidades nos itens do catÃ¡logo acima.</p></div>`;
...
let htmlForn = `
       <div class="flex justify-between items-center mb-2 border-b border-blue-200 pb-1">
         ...
         <div class="flex gap-4 text-[10px] font-bold text-blue-400 uppercase tracking-wider text-right pr-1">
           <span class="w-16">Qtd</span><span class="w-20">Bruto</span><span class="w-20">LÃ­quido</span>
         </div>
       </div>`;
...
itemLinha.innerHTML = `
         <div class="flex items-center gap-3 min-w-0">
           ...
           <div class="min-w-0">
             ...
             <p class="text-xs text-slate-500 truncate">${escapeHtml(p.descricao || 'Sem descriÃ§Ã£o')}</p>
...
function _montarGaleriaImagens(p) {
  ...
  return [...new Set([...principal, ...extras])]; // remove duplicadas, mantÃ©m ordem
}
...
function _renderizarGaleria() {
  ...
  // Se o zoom estiver aberto, mantÃ©m a imagem ampliada sincronizada
...
document.getElementById('modalDescricao').innerText = p.descricao || 'Sem descriÃ§Ã£o cadastrada';
...
if (temDesconto) {
  ...
  textoEconomia.innerText = `VocÃª economiza: ${formatarParaReal(precoOriginal - precoFinal)} por unidade`;
...
// Bloco ST usa o mesmo cÃ¡lculo central aplicado em cards, carrinho e exportaÃ§Ãµes.
...
if (estoque > 0) {
badge.innerText = `DisponÃ­vel: ${estoque} un`;
...
corpo.innerHTML = '<div class="text-center py-10 text-slate-400 text-sm font-medium">Nenhum histÃ³rico de faturamento encontrado para este cliente.</div>';
return;
}
corpo.innerHTML = resp.fornecedores.map(f => `
   <div class="bg-white border border-slate-200 rounded-xl p-3">
     <div class="flex items-center justify-between mb-2">
       <span class="text-xs font-black text-slate-700">${escapeHtml(f.fornecedor)}</span>
       ${f.positivado
         ? '<span class="text-[9px] font-black text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">âœ“ Positivado</span>'
         : '<span class="text-[9px] font-black text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">â€” Sem compra no mÃªs</span>'}
     </div>
     <div class="grid grid-cols-3 gap-2 text-[11px]">
       ...
       <div><span class="block text-slate-400 text-[9px] uppercase font-bold">MÃªs Atual</span><span class="font-bold text-orange-600">${formatarParaReal(f.faturadoMesAtual)}</span></div>
     </div>
   </div>`).join('');
}
...
// CONSTRÃ“I O WORKBOOK FORMATADO (compartilhado pelo carrinho normal
// e pela tela de importaÃ§Ã£o de pedido por Excel/PDF)
// dados = { itensPorFornecedor, clienteInfo, ufExibicao, olAtivo,
//           acBruto, acLiquido, acUnidades, totalVariedades, subtitulo,
//           acQtdAtendida, acValorAtendido, acQtdNaoAtendida, acValorNaoAtendido } <- opcionais (sÃ³ importaÃ§Ã£o)
...
workbook.creator = 'HBN1 - NazÃ¡ria Distribuidora FarmacÃªutica';
...
// --- CabeÃ§alho principal (banner laranja) ---
ws.mergeCells('A1:I3');
...
{ font: { bold: true, size: 10, color: { argb: 'FFFFE8D6' }, name: 'Calibri' }, text: (subtitulo || 'COMPROVANTE DE PEDIDO â€” CATÃLOGO HBN1') + '\n' },
{ font: { size: 9, color: { argb: 'FFFFE8D6' }, name: 'Calibri' }, text: `UF: ${ufExibicao} â€¢ ${dataHoraAgora} â€¢ ${totalVariedades} variedade(s)` }
]
};
...
adicionarLinhaInfo('Cliente', 'NENHUM CLIENTE SELECIONADO / NÃƒO IDENTIFICADO');
}
linhaAtual++;
...
adicionarLinhaInfo('Valor Total LÃ­quido', formatarParaReal(acLiquido));
linhaAtual++;

// --- Resumo de verificaÃ§Ã£o de estoque (somente quando vindo da importaÃ§Ã£o) ---
if (temResumoEstoque) {
mesclarEEstilizar(linhaAtual, 'VERIFICAÃ‡ÃƒO DE ESTOQUE', { corFundo: LARANJA });
linhaAtual++;
adicionarLinhaInfo('âœ… Atendido (qtd / valor)', `${acQtdAtendida} un  â€”  ${formatarParaReal(acValorAtendido)}`);
if (acQtdNaoAtendida > 0) {
const lblCel = ws.getCell(`A${linhaAtual}`);
lblCel.value = 'âš ï¸ NÃ£o Atendido (qtd / valor)';
lblCel.font = { bold: true, size: 10, color: { argb: VERMELHO_SEM } };
ws.mergeCells(`B${linhaAtual}:I${linhaAtual}`);
const valCel = ws.getCell(`B${linhaAtual}`);
valCel.value = `${acQtdNaoAtendida} un  â€”  ${formatarParaReal(acValorNaoAtendido)}`;
...
const cabecalhosColunas = ['COD.', 'EAN', 'DESCRIÃ‡ÃƒO', 'PREÃ‡O', 'DESC%', 'PR.FINAL', 'QT', 'SUBTOTAL', 'STATUS'];
...
// Vem do fluxo de importaÃ§Ã£o: status reflete o atendimento real (total, parcial ou nenhum)
...
rodapeCel.value = `HBN1 - NazÃ¡ria Distribuidora FarmacÃªutica  â€¢  Gerado em ${dataHoraAgora}`;
...
// Aciona o download de um workbook jÃ¡ pronto
...
// =========================================================================
// IMPORTAÃ‡ÃƒO DE PEDIDO POR ARQUIVO (EXCEL/PDF) â€” MÃšLTIPLOS CNPJs NO MESMO ARQUIVO
// =========================================================================
...
let PDF_EXTRACAO_MODO = 'heuristica'; // 'heuristica' | 'ia' â€” sÃ³ ADMIN pode trocar pra 'ia'
...
if (ehPedidoUnilever) {
  ...
  if (TIPO_USUARIO !== 'ADMIN') {
    throw new Error('Envio de imagem disponÃ­vel apenas para administradores.');
  }

  if (file.size > 8 * 1024 * 1024) {
    throw new Error('A imagem deve ter no mÃ¡ximo 8 MB.');
  }
  ...
  throw new Error(
    'Formato nÃ£o suportado. Envie .xlsx, .xls, .pdf, .jpg ou .png.'
  );
}

    if (itensBrutos.length === 0) {
      throw new Error('NÃ£o foi possÃ­vel identificar produtos no arquivo. Verifique se o arquivo contÃ©m os cÃ³digos EAN e as quantidades dos produtos.');
    }

    await processarItensImportados(itensBrutos);
  } catch (erro) {
    logDiagnostico('error', 'Erro ao importar pedido:', erro);
    mostrarErroImportacao(erro.message || 'Ocorreu um erro ao ler o arquivo. Tente novamente.');
  }
}
// 2) LEITURA DE EXCEL (.xlsx/.xls) â†’ array de linhas (cada linha = array de cÃ©lulas)
...
resolve(todasLinhas);
} catch (erro) { reject(new Error('NÃ£o foi possÃ­vel ler o arquivo Excel. Ele pode estar corrompido.')); }
};
reader.onerror = () => reject(new Error('Falha ao carregar o arquivo Excel.'));
reader.readAsArrayBuffer(file);
});
}
// 3) LEITURA DE PDF â†’ array de linhas de texto (agrupadas por posiÃ§Ã£o vertical)
...
async function extrairItensArquivoViaIA(file, mimeType) {
  if (TIPO_USUARIO !== 'ADMIN') {
    throw new Error('ExtraÃ§Ã£o por IA disponÃ­vel apenas para administradores.');
  }
  ...
  if (!resposta || resposta.erro) {
    throw new Error(
      ...
      'A IA nÃ£o conseguiu processar o arquivo.'
    );
  }
...
// 4) EXTRAÃ‡ÃƒO DE ITENS (CNPJ + CÃ“DIGO/EAN + QTD) A PARTIR DAS LINHAS
//    Funciona tanto para linhas do Excel (cÃ©lulas) quanto do PDF (tokens)
// Normaliza CNPJ para 14 dÃ­gitos (padding de zeros Ã  esquerda)
function normalizarCNPJ(digitos) {
return String(digitos).replace(/\D/g, '').padStart(14, '0');
}

// Normaliza EAN para 8 ou 13 dÃ­gitos (padding de zeros Ã  esquerda)
...
// Detecta se um token Ã© um CNPJ vÃ¡lido.
// Aceita: formato pontuado (XX.XXX.XXX/XXXX-XX) com 13 ou 14 dÃ­gitos,
//         ou string de exatamente 14 dÃ­gitos numÃ©ricos.
// NÃƒO aceita strings de 13 dÃ­gitos puros (seriam EAN-13).
function isCNPJ(token) {
const s = String(token).trim();
const d = s.replace(/\D/g, '');
// Formato pontuado com / e - â†’ CNPJ mesmo com zero cortado (13 dÃ­gitos)
if (/\//.test(s) && /-/.test(s) && (d.length === 13 || d.length === 14)) return true;
// String de exatamente 14 dÃ­gitos
...
// CNPJ da prÃ³pria NazÃ¡ria (distribuidora) â€” nunca deve ser tratado como CNPJ
// do cliente, mesmo que apareÃ§a no meio do arquivo (ex: linha "Fornecedor ...").
const CNPJS_IGNORAR_COMO_CLIENTE = ['07224991002189'];
// Linhas de metadado do pedido (cabeÃ§alho/rodapÃ© entre um CNPJ e outro) nunca
// devem ser lidas como linha de produto â€” mesmo que por acaso contenham um
// nÃºmero que pareÃ§a cÃ³digo/quantidade (ex: "CÃ³digo: 1.749.801 Status: ...").
const MARCADORES_METADADO_PEDIDO = ['CÃ“DIGO:', 'STATUS:', 'USUÃRIO:', 'UNIDADE:', 'COTAÃ‡ÃƒO:', 'IMPRESSÃƒO:', 'COND.', 'DATA'];
...
// decimais (preÃ§o unitÃ¡rio x quantidade â‰ˆ total). Mais confiÃ¡vel que pegar
// "o Ãºltimo nÃºmero curto da linha", jÃ¡ que layouts de pedido costumam ter
// vÃ¡rias colunas numÃ©ricas curtas (cÃ³digo do fabricante, giro de estoque,
// qtd por caixa etc.) que nÃ£o sÃ£o a quantidade pedida.
//
// IMPORTANTE: nÃ£o assume qual decimal vem primeiro/depois no texto extraÃ­do
// (a ordem dos tokens lidos do PDF pode nÃ£o seguir a ordem visual da tabela).
// Em vez disso usa o MENOR decimal como preÃ§o e o MAIOR como total â€” isso Ã©
// sempre verdade matematicamente (total = preÃ§o Ã— qtd, e qtd â‰¥ 1), entÃ£o
// funciona independente da ordem em que o PDF entregou os tokens.
function extrairQuantidadeConfiavel(tokens, candidatosQtd) {
// Identifica os Ã­ndices de tokens que sÃ£o percentual de desconto â€” ou seja,
// o valor decimal que aparece IMEDIATAMENTE ANTES de um token "%" (ex: "10,00 %").
// Isso nunca pode ser preÃ§o nem total, e Ã© excluÃ­do independente do valor
// (antes sÃ³ excluÃ­a "0,00", o que deixava passar descontos como "10,00%", "5,00%" etc.
// e contaminava o cÃ¡lculo min/max, gerando quantidades erradas).
...
if (preco > 0 && total >= preco) {   // >= cobre qtd=1, onde preÃ§o == total
const qtdCalculada = total / preco;
// Tenta achar um candidato curto (1-9999) que bata com o cÃ¡lculo (tolerÃ¢ncia Â±0.6)
...
// Se nenhum candidato bateu mas o resultado Ã© inteiro plausÃ­vel, usa direto
if (Math.abs(qtdCalculada - Math.round(qtdCalculada)) < 0.05 && qtdCalculada >= 1 && qtdCalculada < 100000) {
return Math.round(qtdCalculada);
}
}
}
// Fallback: sem cruzamento confiÃ¡vel â€” usa o Ãºltimo nÃºmero curto da linha.
// IMPORTANTE: exclui candidatos que parecem cÃ³digo interno NAZARIA (aparecem logo
// apÃ³s o EAN, antes de qualquer decimal na linha) para nÃ£o confundir com quantidade.
...
// ===================================================================
// CONFIGURAÃ‡ÃƒO DE COLUNAS POR FORMATO DE PDF
// PDF_TIPO_1 â†’ PDFs "bem estruturados" onde o pdf.js entrega cada
//   linha como um array de cÃ©lulas jÃ¡ separadas (ex: planilha NAZARIA).
//   A quantidade Ã© lida diretamente da coluna cujo cabeÃ§alho bate com
//   PDF1_COL_QTD (busca por palavras, nÃ£o sensÃ­vel a maiÃºsculas).
//
// PDF_TIPO_2 â†’ PDFs onde o pdf.js junta tudo numa mesma linha de texto
//   (ex: A7 Pharma / Kenvue). Nesses casos nÃ£o hÃ¡ separaÃ§Ã£o por coluna;
//   a quantidade Ã© localizada como o primeiro nÃºmero curto que aparece
//   APÃ“S o token marcador PDF2_TOKEN_APOS_QTD (que, no padrÃ£o Kenvue,
//   Ã© o sÃ­mbolo "%" do campo "Desc. (%)").
//
// âžœ Se receber um PDF de outro sistema, basta trocar:
//      PDF1_COL_QTD  â†’ nome (ou parte do nome) do cabeÃ§alho da coluna
//      PDF2_TOKEN_APOS_QTD â†’ token que aparece IMEDIATAMENTE antes da
//                            quantidade na linha concatenada do pdf.js
// ===================================================================

// --- Formato 1 (NAZARIA / planilha estruturada) ---
// Palavras aceitas no cabeÃ§alho da coluna de quantidade.
// A busca Ã© por inclusÃ£o, nÃ£o por igualdade exata.
const PDF1_COL_EAN     = ['CÃ³d. Barras','CODBARRAS', 'EAN', 'BARRAS'];
const PDF1_COL_CODIGO  = ['CÃ“D.', 'COD.', 'CÃ“DIGO', 'CODIGO', 'COD'];
const PDF1_COL_QTD     = ['PEDIDA', 'QT PEDIDA', 'QTDE', 'QUANTIDADE', 'QUANT', 'QTD', 'QT'];
// Obs.: 'PEDIDA' captura "QT Pedida" sem precisar de match exato.

// --- Formato 2 (A7 Pharma / Kenvue â€” texto concatenado) ---
// Token que aparece LOGO ANTES da quantidade na linha.
// No padrÃ£o Kenvue a ordem Ã©: ... PREÃ‡O  0,00  %  QTD  TOTAL  FABRICANTE
// â†’ o "%" separa o desconto da quantidade.
const PDF2_TOKEN_APOS_QTD = '%';
// Detecta se o PDF Ã© do Tipo 1 (cabeÃ§alho de tabela identificÃ¡vel).
// Retorna { linhaCab, idxCodigo, idxQtd } ou null.
//
// IMPORTANTE: NÃƒO exige que CNPJ esteja na mesma linha do cabeÃ§alho.
// No NAZARIA o cabeÃ§alho da tabela Ã©:
//   "CÃ³d.FÃ¡b. | Codbarras | CÃ³digo | DescriÃ§Ã£o | ... | QT Pedida | ..."
// e o CNPJ do cliente aparece em linhas separadas do cabeÃ§alho do pedido.
// O CNPJ Ã© lido linha a linha dentro de extrairItensTipo1.
//
// O pdf.js Ã s vezes divide o cabeÃ§alho em 2 linhas (ex: "QT" numa linha
// e "Pedida" na seguinte). Por isso a busca olha tambÃ©m a linha seguinte
// ao Ã­ndice corrente ao procurar pela coluna de quantidade.
...
// TIPO 1 â€” PDF estruturado (ex: NAZARIA)
// LÃª a quantidade diretamente da cÃ©lula na coluna PDF1_COL_QTD.
// O CNPJ do cliente aparece nas linhas de cabeÃ§alho de cada pedido,
// ANTES da linha de itens. Por isso fazemos dois passes:
//   1) PrÃ©-carrega o CNPJ do primeiro cliente varrendo as linhas
//      anteriores ao cabeÃ§alho da tabela.
...
// Passo 1: prÃ©-carrega CNPJ lendo as linhas ANTES do cabeÃ§alho
...
// Passo 2: percorre as linhas de dados apÃ³s o cabeÃ§alho
// Passo 2: percorre as linhas de dados apÃ³s o cabeÃ§alho
...
// Nunca tenta extrair produto de linha de metadado (CÃ³digo:/Status:/UsuÃ¡rio: etc.)
...
// Valida se o valor posicional Ã© mesmo um EAN puro (6-13 dÃ­gitos, sem pontuaÃ§Ã£o).
// Se nÃ£o for (coluna desalinhada por quebra de linha do Produto/Fabricante),
...
// TIPO 2 â€” PDF de texto concatenado (ex: A7 Pharma / Kenvue)
// O pdf.js junta vÃ¡rios campos numa sÃ³ linha de tokens.
// EstratÃ©gia:
//   â€¢ CNPJ do cliente: primeiro CNPJ encontrado na linha (ignora
//     linhas com a palavra "FORNECEDOR")
//   â€¢ CÃ³digo (EAN): primeiro token com 7-13 dÃ­gitos
//   â€¢ Quantidade: primeiro nÃºmero curto (1-4 dÃ­gitos) que aparece
//     APÃ“S o token PDF2_TOKEN_APOS_QTD (padrÃ£o: "%")
...
async function extrairItensPedidoUnileverPorPagina(file) {
  ...
  for (let pagina = 1; pagina <= doc.numPages; pagina++) {
    ...
    // MantÃ©m a ordem natural em que o PDF gravou os textos.
    ...
    // PadrÃ£o do pedido:
    // ... descriÃ§Ã£o + EAN + quantidade pedida + preÃ§o unitÃ¡rio
    //
    // Exemplos:
...
//    da UF de cada cliente, casa itens e calcula preÃ§os
...
// Mapa de UFs disponÃ­veis para o seletor manual
const MAPA_UFS_DISPONIVEIS = {
'PI': 'PiauÃ­', 'MA': 'MaranhÃ£o/Timon', 'IMPTZ': 'Imperatriz',
'TO': 'Tocantins', 'PB': 'ParaÃ­ba', 'PE': 'Pernambuco',
'PA': 'ParÃ¡', 'AP': 'AmapÃ¡', 'RN': 'Rio Grande do Norte',
'CE': 'CearÃ¡', 'BA': 'Bahia', 'SE': 'Sergipe', 'AL': 'Alagoas'
};
...
mostrarToast('error', 'Itens originais nÃ£o disponÃ­veis. Tente reimportar o arquivo.');
return;
}
...
if (!p && eanDigits) p = produtosDaUF.find(prod => String(prod.id).trim() === eanDigits); // fallback: Ã s vezes o "EAN" do arquivo do cliente Ã© na verdade o cÃ³digo interno
...
// Arquivo sem CNPJ: vai direto para seleÃ§Ã£o manual de UF
...
// VerificaÃ§Ã£o de estoque: quanto desta linha o estoque atual do catÃ¡logo consegue atender
...
// 6) RENDERIZAÃ‡ÃƒO DO RESULTADO NA TELA
...
mostrarErroImportacao('Nenhum item pÃ´de ser processado. Verifique o arquivo e tente novamente.');
return;
}
...
const blocoEstoqueClienteHtml = acQtdNaoAtendida > 0 ? `
           <div class="bg-white px-3 py-2 border-t border-emerald-100 flex flex-wrap gap-x-5 gap-y-1 text-[11px]">
             <span class="font-bold text-emerald-700">âœ… Atendido: ${acQtdAtendida} un â€¢ ${formatarParaReal(acValorAtendido)}</span>
             <span class="font-bold text-red-600">âš ï¸ NÃ£o atendido: ${acQtdNaoAtendida} un â€¢ ${formatarParaReal(acValorNaoAtendido)}</span>
           </div>` : '';
         ...
         <div class="border border-emerald-200 rounded-2xl overflow-hidden">
           <div class="bg-emerald-50 p-3 flex flex-wrap items-center justify-between gap-2">
             <div>
               ...
               <p class="text-[11px] text-emerald-700 font-mono">${formatarCNPJ(cliente.cnpj ? normalizarSoDigitos(cliente.cnpj) : '')} â€¢ UF: ${cliente.uf || '-'}${olDetectado > 0 ? ' â€¢ OL Danone: ' + olDetectado : ''}</p>
             </div>
             <div class="text-right">
               <p class="text-[9px] font-black text-emerald-600 uppercase tracking-wider">Total LÃ­quido</p>
               <p class="text-sm font-black text-emerald-800">${formatarParaReal(acLiquido)}</p>
               <p class="text-[10px] text-emerald-600">${totalVariedades} itens â€¢ ${acUnidades} un</p>
             </div>
           </div>
...
blocoResumoEstoque.innerHTML = `
         <div class="bg-emerald-50 rounded-xl p-3 border-2 border-emerald-200 text-center">
           <span class="text-[9px] font-black text-emerald-600 uppercase tracking-wider block">âœ… Valor Atendido</span>
         ...
         <div class="bg-red-50 rounded-xl p-3 border-2 border-red-200 text-center">
           <span class="text-[9px] font-black text-red-600 uppercase tracking-wider block">âš ï¸ Valor NÃ£o Atendido</span>
...
blocoResumoEstoque.innerHTML = `
         <div class="col-span-2 bg-emerald-50 rounded-xl p-3 border-2 border-emerald-200 text-center">
           <span class="text-xs font-black text-emerald-700">âœ… Estoque suficiente para atender 100% do pedido (${acQtdAtendidaGeral} un)</span>
         </div>`;
}
         ...
         <div class="bg-white rounded-xl border border-amber-200 p-3 flex flex-col gap-2">
           <div class="flex items-center justify-between flex-wrap gap-2">
             <div>
               ...
               <span class="text-amber-500 text-[10px] ml-2">${n.linhas} linha(s) â€¢ ${n.unidades} un</span>
             </div>
             <span class="text-[9px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">CNPJ nÃ£o cadastrado</span>
           </div>
...
if (codigosNaoEncontrados.length > 0 || temPedidosDescartados) {
  ...
  if (temPedidosDescartados) {
    textoFinal += `âš  ${pedidosSemProdutoNoCatalogo.length} pedido(s) descartado(s) â€” nenhum item pertence ao seu catÃ¡logo:\n`;
    textoFinal += pedidosSemProdutoNoCatalogo.map(p =>
      `  â€¢ ${(p.cliente.razao || '').toUpperCase()} (${p.cnpjFormatado}) â€” ${p.totalItensDoArquivo} item(ns), ${p.unidades} un`
    ).join('\n');
    textoFinal += codigosNaoEncontrados.length > 0 ? '\n\n' : '';
  }
  if (codigosNaoEncontrados.length > 0) {
    textoFinal += 'CÃ³digos/EANs nÃ£o localizados: ' + codigosNaoEncontrados.join(', ');
  }
...
// 7) EXPORTAÃ‡ÃƒO EM EXCEL DO PEDIDO IMPORTADO (todos os CNPJs no mesmo arquivo)

// O Excel nÃ£o permite duas abas com o mesmo nome no mesmo arquivo. Como Ã© comum
// que dois CNPJs diferentes pertenÃ§am Ã  mesma razÃ£o social (filiais de uma rede,
// ou nomes que ficam iguais apÃ³s o corte de caracteres), esta funÃ§Ã£o garante que
// cada aba tenha um nome Ãºnico, acrescentando " (2)", " (3)" etc. quando necessÃ¡rio.
function gerarNomeAbaUnico(nomeBase, nomesJaUsados) {
const limite = 31; // limite mÃ¡ximo de caracteres permitido pelo Excel para nomes de aba
...
acQtdAtendida, acValorAtendido, acQtdNaoAtendida, acValorNaoAtendido,
totalVariedades,
subtitulo: 'PEDIDO IMPORTADO â€” ARQUIVO DO CLIENTE'
});
...
const wsNaoEnc = workbook.addWorksheet('CNPJ NÃ£o Encontrados', { views: [{ showGridLines: false }] });
wsNaoEnc.columns = [{ width: 22 }, { width: 14 }, { width: 14 }];
wsNaoEnc.mergeCells('A1:C1');
const titulo = wsNaoEnc.getCell('A1');
titulo.value = 'CNPJs NÃƒO ENCONTRADOS NA BASE DE CLIENTES';
...
logDiagnostico('error', 'Erro ao gerar Excel do pedido importado:', erro);
  ...
  (IMPORT_RESULTADO.pedidos || []).forEach(pedido => {
    ...
    Object.entries(pedido.itensPorFornecedor || {}).forEach(([fornecedor, itens]) => {
      (itens || []).forEach(item => {
        ...
        // Fornecedor + cÃ³digo + EAN evita juntar produtos diferentes por engano.
...
async function baixarExcelPedidoImportadoConsolidado() {
  if (TIPO_USUARIO !== 'ADMIN') {
    mostrarToast('warning', 'FunÃ§Ã£o disponÃ­vel apenas para administradores.');
    return;
  }
  ...
  try {
    ...
    if (itens.length === 0) {
      mostrarToast('warning', 'NÃ£o hÃ¡ itens encontrados no catÃ¡logo para compilar.');
      return;
    }
    ...
    titulo.value = 'PEDIDO CONSOLIDADO â€” TODOS OS CNPJs';
    ...
    const cabecalhos = [
      'FORNECEDOR', 'CÃ“DIGO', 'EAN', 'DESCRIÃ‡ÃƒO', 'MARCA', 'EMBALAGEM',
      'UF(s)', 'QTD. TOTAL', 'CNPJs', 'VALOR BRUTO', 'VALOR LÃQUIDO', 'PREÃ‡O LÃQ. MÃ‰DIO'
    ];
    ...
    logDiagnostico('error', 'Erro ao compilar pedidos importados:', erro);
    mostrarToast('error', 'NÃ£o foi possÃ­vel gerar o pedido consolidado.');
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.style.opacity = '';
...
// EXPORTAÃ‡ÃƒO DO CARRINHO ATUAL (botÃ£o "Fechar & Baixar Pedido")
...
function _idsHistoricoRegistrados() {
  try { return JSON.parse(sessionStorage.getItem(CHAVE_HISTORICO_REGISTRADO) || '[]'); }
  catch { return []; }
}
...
function _marcarHistoricoRegistrado(pedidoId) {
  ...
  // MantÃ©m apenas os registros recentes para nÃ£o crescer indefinidamente.
  sessionStorage.setItem(CHAVE_HISTORICO_REGISTRADO, JSON.stringify(ids.slice(0, 300)));
}
...
async function _registrarItensNoHistorico(pedidoId, cliente, itensOrigem, olAtivo) {
  ...
  const tarefas = Object.keys(itensPorFornecedor).map(forn => {
    ...
    return chamarApi('registrarHistoricoPedido', {
      dados: {
        ...
        olAtivo: Number(OL_ATIVO) || 0,
        omronOlAtivo: Number(OMRON_OL_ATIVO) || 0,
  ...
  try {
    ...
    logDiagnostico('error', 'Erro ao registrar pedido no histÃ³rico:', erro);
    mostrarToast('warning', 'O arquivo foi gerado, mas o histÃ³rico nÃ£o pÃ´de ser atualizado agora.');
    return false;
  }
}
    ...
    overlay.innerHTML = `
      <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
        <div class="p-6 border-b border-slate-100">
          ...
          <p class="text-xs text-slate-500 mt-1">Escolha o catÃ¡logo que o cliente poderÃ¡ visualizar.</p>
        </div>
        <div class="p-5 space-y-3">
          <button id="ofertaEscopoCompleto" class="w-full text-left p-4 rounded-2xl border border-slate-200 hover:border-orange-300 hover:bg-orange-50 transition-colors">
            <span class="block text-sm font-black text-slate-800">CatÃ¡logo completo</span>
            <span class="block text-xs text-slate-500 mt-1">O cliente poderÃ¡ navegar por todos os fornecedores.</span>
          </button>
          <button id="ofertaEscopoFornecedor" class="w-full text-left p-4 rounded-2xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 transition-colors">
            ...
            <span class="block text-xs text-slate-500 mt-1">O cliente verÃ¡ somente os produtos deste fornecedor.</span>
          </button>
...
// =========================================================================
// OFERTA COMPARTILHADA â€” vendedor gera um link temporÃ¡rio pro cliente
// selecionado montar o prÃ³prio pedido sozinho e devolver via WhatsApp.
// =========================================================================
async function compartilharOfertaComCliente() {
  ...
  try {
    ...
    if (!resp || resp.erro || !resp.token) {
      mostrarToast('error', (resp && resp.mensagem) || 'NÃ£o foi possÃ­vel gerar o link.');
      return;
    }

    const url = `${window.location.origin}${window.location.pathname.replace(/[^/]*$/, '')}catalogo.html#oferta=${encodeURIComponent(resp.token)}`;
    const nomeCliente = resp.clienteNome || CLIENTE_SELECIONADO.razao || 'vocÃª';
    const descricao = fornecedor ? `o catÃ¡logo de ${fornecedor}` : 'o catÃ¡logo completo';
    const mensagem = `OlÃ¡! Segue ${descricao} para vocÃª, ${nomeCliente}. Ã‰ sÃ³ montar o pedido por aqui: ${url}`;

    if (navigator.share) {
      try {
        await navigator.share({ title: 'CatÃ¡logo HBN1', text: mensagem });
        return;
      } catch (erroCompartilhar) {
        if (erroCompartilhar && erroCompartilhar.name !== 'AbortError') {
          logDiagnostico('warn', 'Compartilhamento nativo indisponÃ­vel:', erroCompartilhar);
        }
      }
    }
    window.open('https://wa.me/?text=' + encodeURIComponent(mensagem), '_blank');
  } catch (erro) {
    logDiagnostico('error', 'Erro ao compartilhar oferta:', erro);
...
subtitulo: 'COMPROVANTE DE PEDIDO â€” CATÃLOGO HBN1'
});
...
logDiagnostico('error', 'Erro ao gerar o Excel do pedido:', erro);
...
// =========================================================================
// ST â€” SUBSTITUIÃ‡ÃƒO TRIBUTÃRIA (inline na barra de cliente)
// =========================================================================
...
// SÃ³ vira exigÃªncia de faturamento mÃ­nimo se o valor for > 0.
// Linhas com mÃ­nimo zerado existem sÃ³ pra controlar validade
...
.catch(e => logDiagnostico('error', 'Valores mÃ­nimos nÃ£o disponÃ­veis:', e));
}
// =========================================================================
// FATURAMENTOS MÃNIMOS â€” cÃ¡lculo, indicador inline e modal de alerta
// =========================================================================

// Calcula o status de cada grupo de desconto com mÃ­nimo configurado.
...
// Atualiza o botÃ£o/pÃ­lula de mÃ­nimos na barra de controles
...
? `âœ… <span class="font-semibold">${atingidos[0].label}</span> <span class="font-medium opacity-75">â€¢ MÃ­nimo atingido</span>`
: `âœ… <span class="font-semibold">${total} mÃ­nimos atingidos</span>`;
...
btn.innerHTML = `âš  <span class="font-semibold">${p.label}</span> <span class="font-medium opacity-75">â€¢ MÃ­n: ${formatarParaReal(p.minimo)} â€¢ Faltam ${formatarParaReal(p.faltam)}</span>`;
} else {
// MÃºltiplos ou misturados â€” mostra resumo + seta para o popover
...
? `âš  <span class="font-semibold">${pendentes[0].label}</span> <span class="font-medium opacity-75">â€¢ Faltam ${formatarParaReal(pendentes[0].faltam)}</span> ${chevron}`
: `âš  <span class="font-semibold">${pendentes.length} faturamentos mÃ­nimos pendentes</span> ${chevron}`;
}
}

// Abre/fecha o popover com o detalhamento de todos os mÃ­nimos
...
pop.innerHTML = `
   <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
     <p class="text-xs font-black text-slate-700 uppercase tracking-wider">Faturamentos MÃ­nimos</p>
       ...
       return `
         <div class="rounded-xl p-3 border ${ok ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'}">
           <div class="flex items-center justify-between gap-2 mb-1.5">
             <span class="text-xs font-black ${ok ? 'text-emerald-700' : 'text-amber-700'}">${ok ? 'ðŸŸ¢' : 'ðŸŸ '} ${item.label}</span>
             ${ok
               ? `<span class="text-[10px] font-black text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-full">âœ“ Atingido</span>`
           ...
           <div class="flex gap-4 text-[10px] font-mono">
             ...
             <span class="text-slate-400">MÃ­nimo: <strong>${formatarParaReal(item.minimo)}</strong></span>
           </div>
         </div>`;
     }).join('')}
   </div>`;
...
// â”€â”€ Modal de alerta de mÃ­nimos (antes de baixar/salvar) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
if (pendentes.length === 0) {
// Sem pendÃªncias â€” executa direto
onProceed();
return;
}
...
? 'âš  Faturamento mÃ­nimo nÃ£o atingido'
: `âš  ${pendentes.length} faturamentos mÃ­nimos pendentes`;
...
? `<p class="text-slate-600 text-xs mb-3 font-semibold">${pendentes[0].label}</p>
      <div class="bg-amber-50 rounded-xl p-3 border border-amber-200 space-y-1.5 text-xs">
        ...
        <div class="flex justify-between"><span class="text-slate-500">MÃ­nimo</span><span class="font-bold text-slate-700">${formatarParaReal(pendentes[0].minimo)}</span></div>
...
: `<p class="text-slate-400 text-xs mb-2">Os seguintes mÃ­nimos ainda nÃ£o foram atingidos:</p>
      <div class="space-y-1.5">
        ${pendentes.map(p => `
          <div class="flex items-center justify-between bg-amber-50 rounded-xl px-3 py-2 border border-amber-100">
            <span class="text-xs font-bold text-amber-700">ðŸŸ  ${p.label}</span>
...
return `
         <div class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border ${atingiu ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}">
           <span class="text-[11px]">${atingiu ? 'âœ…' : 'âŒ'}</span>
...
try { return JSON.parse(sessionStorage.getItem(CHAVE_PEDIDOS_SALVOS) || '[]'); }
catch { return []; }
}

function persistirPedidosSalvos(lista) {
  try {
    sessionStorage.setItem(CHAVE_PEDIDOS_SALVOS, JSON.stringify(lista));
    return true;
  } catch (e) {
    logDiagnostico('error', 'Falha ao salvar pedidos:', e);
    mostrarToast(
      'error',
      'NÃ£o foi possÃ­vel salvar o pedido â€” armazenamento local cheio. Tente limpar o cache de imagens do catÃ¡logo e salve novamente.',
      6000
    );
    return false;
  }
...
const chaves = Object.keys(CARRINHO);
if (chaves.length === 0) { mostrarToast('warning', 'Seu carrinho estÃ¡ vazio. Adicione produtos antes de salvar.'); return; }
...
// Se falhou, nÃ£o limpa o carrinho â€” o usuÃ¡rio nÃ£o perde o pedido montado.
...
// MantÃ©m o botÃ£o flutuante visÃ­vel durante o feedback, mesmo com carrinho jÃ¡ vazio
const btnFlutuante = document.getElementById('btnSalvarPedidoFlutuante');
if (btnFlutuante) btnFlutuante.classList.remove('hidden');

// Feedback visual nos botÃµes de salvar (modal + flutuante)
           ...
           ${itens.map(item => {
             ...
             return `<div class="flex items-start justify-between gap-2 py-1.5 border-b border-slate-50 last:border-0">
               <div class="flex-grow min-w-0">
                 ...
                 <p class="text-[9px] text-slate-400 mt-0.5">${escapeHtml(item.marca || '')} Â· ${escapeHtml(item.embalagem || '')}</p>
               </div>
               <div class="text-right shrink-0">
                 <p class="text-[10px] font-black text-slate-800">${item.qtd} un Ã— ${precoFmt} ${descTag}</p>
           ...
           <div id="preview_${p.id}" class="hidden border-t border-slate-100 bg-slate-50/60 px-4 py-3 max-h-72 overflow-y-auto no-scrollbar">
             <div class="flex items-center justify-between mb-2">
               ...
               <div class="flex gap-3 text-[10px] text-slate-400">
                 ...
                 <span>LÃ­quido: <strong class="text-emerald-700">${liquido}</strong></span>
               </div>
             </div>
             ${linhasPreview}
           </div>
...
mostrarConfirm(
'Remover este pedido?',
'O pedido serÃ¡ removido permanentemente da lista de salvos.',
() => {
...
// Busca o estoque ATUAL do produto no catÃ¡logo â€” o pedido salvo nÃ£o guarda
// isso, entÃ£o sem essa busca a coluna Status sempre saÃ­a "Sem estoque".
...
// Se o produto nÃ£o existir mais no catÃ¡logo (removido/renomeado), nÃ£o dÃ¡
// pra confirmar falta de estoque â€” assume disponÃ­vel em vez de acusar errado.
...
// ... resto da funÃ§Ã£o continua igual (workbook, nome do arquivo, download)
...
logDiagnostico('error', 'Erro ao gerar Excel do pedido salvo:', e);
...
if (Object.keys(CARRINHO).length > 0) {
e.preventDefault();
e.returnValue = 'VocÃª tem itens no seu pedido. Se sair ou atualizar a pÃ¡gina, seu progresso serÃ¡ perdido!';
return e.returnValue;
}
});
window.addEventListener('DOMContentLoaded', async () => {
  await aguardarSessaoInicial();
// Sincroniza Ã­cone do dark mode
renderizarSeletorUF();
atualizarControleTema(document.documentElement.getAttribute('data-theme') || 'light');

// Cliente (Independente, Rede ou Oferta Compartilhada) sÃ³ vÃª o prÃ³prio pedido â€” nada de gestÃ£o/vendas
if (TIPO_USUARIO === 'CLIENTE_INDEPENDENTE' || TIPO_USUARIO === 'CLIENTE_REDE' || TIPO_USUARIO === 'CLIENTE_OFERTA') {
  ...
  // Campanhas Mensais Ã© exclusivo do Dedicado (e Admin)
...
// Oferta compartilhada: alÃ©m das restriÃ§Ãµes de cliente acima, esconde
// tambÃ©m o que nem pro Cliente Independente Ã© escondido hoje â€” ferramentas
// que nÃ£o fazem sentido pra quem sÃ³ abriu um link pra montar UM pedido.
...
// â”€â”€ MURAL COM FADE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let _AVISOS_LISTA    = [];
let _AVISOS_IDX      = 0;
...
.catch(e => logDiagnostico('error', 'Erro nos avisos:', e));
}
...
if (!el) return;
el.textContent   = String(_AVISOS_LISTA[_AVISOS_IDX] || '');
el.style.opacity = '1';
...
// Exposta globalmente para os botÃµes de paginaÃ§Ã£o inline (onclick="_irParaAviso(i)")
...
// STALE: jÃ¡ mostra os produtos da Ãºltima visita, sem esperar a rede
BD_PRODUTOS = cache;
const fornecedorSalvo = sessionStorage.getItem('hbn1_fornecedor_ativo');
...
.then(dados => {
if (!Array.isArray(dados)) throw new Error('Resposta de produtos invÃ¡lida.');
dados.sort((a, b) => {
...
if (isPrimeiraCarga) {
// REVALIDATE: atualiza silenciosamente a tela em que o vendedor jÃ¡ estÃ¡
...
.catch(erro => {
logDiagnostico('error', "Erro na busca de produtos:", erro);
const elAtualizacao = document.getElementById('textoUltimaAtualizacao');
if (elAtualizacao) elAtualizacao.innerHTML = `<span class="text-red-500">âš  Falha ao sincronizar dados</span>`;
if (isPrimeiraCarga && BD_PRODUTOS.length === 0) {
document.getElementById('gridProdutos').innerHTML = `<div class="col-span-full text-center py-10 text-red-500 font-bold">Erro ao conectar com o Google Sheets. Tente atualizar a pÃ¡gina.</div>`;
}
});
}
...
.catch(e => logDiagnostico('error', "Erro nos clientes:", e));
}

// Ajusta a barra de cliente do topo conforme o tipo de sessÃ£o logada.
// CLIENTE_INDEPENDENTE: jÃ¡ entra com o prÃ³prio cliente selecionado, sem
// poder buscar/trocar (BD_CLIENTES sempre tem sÃ³ ele mesmo, pelo backend).
// CLIENTE_REDE: mantÃ©m a busca, mas BD_CLIENTES jÃ¡ vem restrita Ã  rede dele.
function aplicarRestricoesClienteLogado() {
  if (TIPO_USUARIO === 'CLIENTE_INDEPENDENTE' || TIPO_USUARIO === 'CLIENTE_OFERTA') {
    ...
    // Remove o botÃ£o "âœ–" do popover â€” cliente independente nÃ£o pode "limpar"
    ...
    if (inputBusca) inputBusca.placeholder = 'ðŸ” Buscar CNPJ da sua rede...';
  }
}
...
// Heartbeat â€” avisa o backend que a sessÃ£o ainda estÃ¡ ativa e detecta
// bloqueio no meio de uma sessÃ£o jÃ¡ aberta.
enviarHeartbeat();
  ...
  chamarApi('heartbeat', {})
    .then(resp => {
      if (resp && resp.bloqueado) {
        ...
        setTimeout(() => {
          sessionStorage.removeItem('hbn1_usuario');
          sessionStorage.removeItem('hbn1_uf');
          sessionStorage.removeItem('hbn1_nome');
          sessionStorage.removeItem('hbn1_login_ts');
          sessionStorage.removeItem('hbn1_session');
          window.location.href = 'index.html';
        }, 2500);
      }
    })
    .catch(() => {}); // silencioso â€” heartbeat nÃ£o deve incomodar o usuÃ¡rio
}
// =========================================================================
...
try {
sessionStorage.setItem('hbn1_negociacao_handoff', JSON.stringify({ itens, cliente: clienteInfo || null }));
} catch (e) {
mostrarToast('error', 'NÃ£o foi possÃ­vel preparar os dados para a Central de Investimento.');
return;
}
...
// Origem 2: um pedido salvo especÃ­fico (nÃ£o leva os outros salvos)
...
// Origem 3: resultado da importaÃ§Ã£o (Ver Meu Valor do Pedido) â€” soma entre CNPJs
// e separa qtd atendida / nÃ£o atendida em linhas diferentes
function enviarImportacaoParaNegociacao() {
if (!IMPORT_RESULTADO || IMPORT_RESULTADO.pedidos.length === 0) return;

const mapa = {}; // chave = cÃ³digo interno do produto
IMPORT_RESULTADO.pedidos.forEach(pedido => {
...
// Substitua o bloco de funÃ§Ãµes de imagem do PDF por este.
// Resolve Amazon/Google Drive tentando direto, mantÃ©m weserv para CDNs compatÃ­veis
// e deixa um ponto claro para proxy prÃ³prio nos domÃ­nios sem CORS, como RaiaDrogasil.
...
const PDF_IMG_PROXY_PROPRIO = '/imagem?url=';
...
async function _obterImagemBase64ComCache(urlOriginal) {
  ...
  // 1) Tenta achar no Cache API (persiste entre sessÃµes, sem custo de localStorage)
  let cache = null;
  try {
    ...
    logDiagnostico('warn', 'Cache API indisponÃ­vel, seguindo sem cache persistente:', e);
  }

  // 2) NÃ£o achou â€” baixa somente pelo proxy do mesmo domÃ­nio.
  // Assim a URL de origem passa por allowlist, limite de tamanho e validaÃ§Ã£o MIME.
  const tentativas = [
    ['proxy seguro', _obterUrlImagemProxyProprio(url), 15000],
  ].filter(([, u]) => !!u);
  ...
  for (const [rotulo, tentativaUrl, timeout] of tentativas) {
    try {
      ...
      // Salva no Cache API pra prÃ³xima vez â€” clone antes, porque o body sÃ³ pode ser lido uma vez
      if (cache) {
        try {
          await cache.put(url, new Response(blob.slice(), { headers: { 'Content-Type': tipo || 'image/jpeg' } }));
        } catch (e) {
          logDiagnostico('warn', 'NÃ£o foi possÃ­vel persistir no Cache API (seguindo sem cache):', e.message);
        }
      }

      return await _blobParaBase64(blob);
    } catch (e) {
      logDiagnostico('warn', `Imagem (${rotulo}) falhou:`, tentativaUrl, e.message);
    }
  }
...
// â”€â”€ UtilitÃ¡rios de cache: limpar e diagnosticar espaÃ§o â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function limparCacheImagensPdf() {
  try {
    await caches.delete(PDF_IMG_CACHE_NAME);
    mostrarToast('success', 'Cache de imagens do catÃ¡logo limpo.');
    await verificarEExibirAvisoCache(); // reavalia â€” deve sumir agora
  } catch (e) {
    mostrarToast('error', 'NÃ£o foi possÃ­vel limpar o cache de imagens.');
  }
}
...
async function _prefetchImagensComConcorrencia(lista, mapaImagens, concorrencia = 3, onProgresso) {
  ...
  async function worker(numeroWorker) {
    await new Promise(r => setTimeout(r, numeroWorker * 150)); // escalona o inÃ­cio
...
function _truncarTexto(texto, maxCaracteres) {
  ...
  return t.slice(0, maxCaracteres - 1).trim() + 'â€¦';
}
...
<span style="background:#eef2ff;color:#3b5aa8;font-weight:700;height:16px;padding:0 6px;border-radius:4px;display:inline-flex;align-items:center;justify-content:center;line-height:16px;box-sizing:border-box;">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
    <div style="font-size:15px;font-weight:800;color:#e8620a;line-height:1.2;white-space:nowrap;">
      ${precoOriginal > 0 ? formatarParaReal(precoFinal) : 'â€”'}
    </div>
...
function _montarDivPaginaPdf(produtosDaPagina, mapaImagens, info, incluirPreco = true) {
  ...
  // Nome do cliente NÃƒO aparece mais no conteÃºdo do PDF â€” sÃ³ no nome do arquivo.
 ...
 div.innerHTML = `
    <div style="background:#FF6B00;padding:16px 34px;position:relative;">
      <h1 style="color:#fff;font-size:20px;font-weight:900;margin:0;letter-spacing:-0.5px;">CATÃLOGO DE PRODUTOS - ${info.fornecedor}</h1>
      <p style="color:#ffe8d6;font-size:11px;margin:6px 0 0;">${subLinha}</p>
      <span style="position:absolute;top:16px;right:34px;color:#ffe8d6;font-size:10px;">PÃ¡gina ${info.numPagina}/${info.totalPaginas}</span>
    </div>
...
// Modal para perguntar se o PDF deve sair com ou sem preÃ§os â€” criado via JS,
// nÃ£o depende de HTML prÃ©-existente na pÃ¡gina. Retorna Promise<true|false|null>
// (null = cancelado)
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:#fff;border-radius:16px;padding:24px;max-width:360px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.25);font-family:'Plus Jakarta Sans', Arial, sans-serif;">
        <h3 style="font-size:15px;font-weight:800;color:#1e293b;margin:0 0 6px;">Exportar catÃ¡logo em PDF</h3>
        <p style="font-size:12px;color:#64748b;margin:0 0 18px;">Deseja incluir os preÃ§os dos produtos no PDF?</p>
        <div style="display:flex;flex-direction:column;gap:8px;">
          <button id="btnPdfComPreco" style="background:#FF6B00;color:#fff;font-weight:700;font-size:13px;padding:10px 14px;border:none;border-radius:10px;cursor:pointer;">Com preÃ§o</button>
          <button id="btnPdfSemPreco" style="background:#f1f5f9;color:#334155;font-weight:700;font-size:13px;padding:10px 14px;border:none;border-radius:10px;cursor:pointer;">Sem preÃ§o</button>
...
async function baixarCatalogoPdf() {
 ...
 if (lista.length === 0) { mostrarToast('warning', 'Nenhum produto para exportar com os filtros atuais.'); return; }
  ...
  if (incluirPreco === null) return; // usuÃ¡rio cancelou
  ...
  try {
    mostrarToast('info', `Preparando catÃ¡logo com ${lista.length} produto(s)... isso pode levar alguns segundos.`, 5000);
...
if (idsSemImagem.length > 0) {
  logDiagnostico('warn', 'Produtos que ficaram SEM IMAGEM no PDF:');
  idsSemImagem.forEach(p => {
    logDiagnostico('warn', `  ID ${p.id} â€” URL cadastrada: ${p.imagens || '(CAMPO VAZIO NA PLANILHA)'}`);
  });
}
    ...
    const nomeVendedor = sessionStorage.getItem('hbn1_nome') || sessionStorage.getItem('hbn1_usuario') || '';
   ...
   const sufixoCliente = (CLIENTE_SELECIONADO && CLIENTE_SELECIONADO.razao)
    ...
    mostrarToast('success', 'CatÃ¡logo em PDF gerado com sucesso.');
  } catch (erro) {
    logDiagnostico('error', 'Erro ao gerar catÃ¡logo em PDF:', erro);
  ...
  verificarEExibirAvisoCache(); // NOVO â€” reflete o uso imediatamente apÃ³s gerar o PDF
}
}
// =========================================================================
// TALÃƒO DE PEDIDO EM BRANCO (Excel com fÃ³rmulas) â€” catÃ¡logo completo do
// fornecedor ativo (respeita os filtros atuais: marca, divisÃ£o, estoque,
// busca), pronto pra preencher a quantidade em campo/offline.
// Reproduz o formato do talÃ£o fÃ­sico: cabeÃ§alho com Apontador/Cliente/
// Fatura MÃ­nima/Prazo, caixa de resumo por Franquia (SUMIFS) e Valor
// Bruto/LÃ­quido calculados por fÃ³rmula (=Qtd Ã— PreÃ§o) linha a linha â€”
// exatamente como a planilha original usada em campo.
//
// COMO ADICIONAR:
// 1) Cole este bloco inteiro em catalogo.js (ex: logo apÃ³s a funÃ§Ã£o
//    baixarCatalogoPdf(), que jÃ¡ Ã© o padrÃ£o mais parecido).
// 2) No catalogo.html, adicione o botÃ£o abaixo ao lado do botÃ£o
//    "Baixar CatÃ¡logo PDF" (mesmo container, mesma barra de controles):
//
...
// Agrupa por FRANQUIA (ou por MARCA quando o fornecedor nÃ£o usa franquia â€”
// fora da Unilever a franquia normalmente vem "GERAL", entÃ£o cai pra marca)
...
async function baixarTalaoEmBranco() {
  ...
  if (filtroFornecedorAtual === 'TODOS') {
    mostrarToast('warning', 'Selecione um fornecedor antes de gerar o talÃ£o.');
    return;
  }
  const lista = PRODUTOS_FILTRADOS.slice(); // respeita busca + marca + divisÃ£o + estoque atuais
  if (lista.length === 0) {
    mostrarToast('warning', 'Nenhum produto para gerar o talÃ£o com os filtros atuais.');
    return;
  }
  if (lista.length > 3000) {
    mostrarToast('warning', `Muitos produtos para um Ãºnico talÃ£o (${lista.length}). Refine os filtros (marca/divisÃ£o) antes de exportar.`);
    return;
  }
  ...
  try {
    ...
    await baixarWorkbook(workbook, nomeArquivo);
    mostrarToast('success', 'TalÃ£o em branco gerado com sucesso.');
  } catch (erro) {
    logDiagnostico('error', 'Erro ao gerar talÃ£o em branco:', erro);
    mostrarToast('error', 'Ocorreu um erro ao gerar o talÃ£o. Tente novamente.');
  } finally {
    if (btn) { btn.disabled = false; if (htmlOriginal !== null) btn.innerHTML = htmlOriginal; }
  }
}
...
async function _construirWorkbookTalao(lista) {
  ...
  banner.value = {
    richText: [
      { font: { bold: true, size: 18, color: { argb: 'FFFFFFFF' } }, text: 'TALÃƒO DE PEDIDO â€” HBN1\n' },
      { font: { bold: true, size: 10, color: { argb: 'FFFFE8D6' } }, text: `FORNECEDOR: ${filtroFornecedorAtual}\n` },
      { font: { size: 9, color: { argb: 'FFFFE8D6' } }, text: `UF: ${UF_USUARIO} â€¢ ${dataHoraAgora} â€¢ ${lista.length} produto(s)${ST_ATIVO ? ' â€¢ PreÃ§o bruto jÃ¡ com ST' : ''}` }
    ]
  };
  ...
  // --- Ãrea de cabeÃ§alho: campos p/ preencher Ã  mÃ£o + caixa de franquias ---
  ...
  camposInfo.forEach(([label, valor], i) => {
    ...
    val.border = { bottom: { style: 'thin', color: { argb: 'FFCBD5E1' } } }; // linha pra escrever Ã  mÃ£o
    ws.getRow(r).height = 16;
  });

  // Caixa de resumo por franquia (colunas K:M), com SUMIFS ligado Ã  tabela â€”
  // atualiza sozinha conforme o vendedor vai preenchendo as quantidades.
  const franquiaHeaderRow = infoLinhaInicio;
  ['FRANQUIA', 'R$ BRUTO', 'R$ LÃQUIDO'].forEach((titulo, i) => {
  ...
  const titulosColunas = ['CÃ“DIGO', 'DESCRIÃ‡ÃƒO', 'EAN', 'DIVISÃƒO', 'FRANQUIA', 'PREÃ‡O BRUTO', 'DESCONTO%', 'PREÃ‡O FINAL', 'ESTOQUE', 'QTD PEDIDO', 'VALOR BRUTO', 'VALOR LÃQUIDO'];
  const linhaCab = ws.getRow(tabelaHeaderRow);
  titulosColunas.forEach((titulo, i) => {
    const cel = linhaCab.getCell(i + 2); // comeÃ§a na coluna B
    cel.value = titulo;
    cel.font = { bold: true, size: 9.5, color: { argb: 'FFFFFFFF' } };
    cel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: LARANJA } };
  ...
  itensOrdenados.forEach((p, idx) => {
    ...
    // Colunas de fÃ³rmula / campo em branco tambÃ©m precisam de fundo + borda
  ...
  totalLabel.value = 'TOTAL GERAL DO TALÃƒO';
  ...
  // Congela o cabeÃ§alho da tabela ao rolar (Ãºtil pra talÃµes longos)
